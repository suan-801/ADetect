...
def render(session: dict):
    ...
    icon = "🔒" if gated else status_icon("완료" if status == "confirmed" else "미실행")
    st.subheader(f"{icon} 타겟분석")

    if gated:
        feature_card(
            "🎯", "이 기능은 시장분석과 브랜드분석 결과를 근거로 핵심 타겟을 추천합니다",
            "잠금 해제 조건 (시장분석 AND 브랜드분석, §6-1)",
        )
        st.write(f"{'✓' if market_ok else '✕'} 시장분석 완료 필요")
        st.write(f"{'✓' if brand_ok else '✕'} 브랜드분석 완료 필요")
    ...
    if session.get("target_status") == "recommended":
        ...
        sample_data_notice()
        st.markdown(f"### 추천 타겟: **{rec['segment']}**")
    ...
    st.success(f"확정 타겟: **{session['confirmed_target']}** (사용자 확정)")
    ...
    with tabs[2]:
        st.caption(f"⚠️ {result['ai_persona']['disclaimer']}")
