#!/usr/bin/env python3
# 경쟁사 Meta 광고 소재 수집 + Gemini 5단계 분석
...
#   4) 자격증명 등록 (둘 중 하나):
#        (A) 권장 — 워크스페이스의 `09_tracking/.env` 파일에 직접 작성 (스크립트가 자동 로드):
#              APIFY_TOKEN=apify_api_...
#              GEMINI_API_KEY=AQ...   # 2026년 새 키는 AQ 로 시작 (옛 AIza 키는 9월 전까지만 동작)
#            → 한 번 등록하면 셸을 닫아도 유지됨. 다른 스킬과도 공유.
#        (B) 임시 — 현재 셸에서만 환경변수 export:
#              export APIFY_TOKEN="apify_api_..."
#              export GEMINI_API_KEY="AQ..."
#
# 사용 (Less is More — URL 1개만 입력)
#   python3 fetch_competitor_ads.py "https://www.facebook.com/ads/library/?...&view_all_page_id=123"
#       └ 첫 크롤 후 page_name 으로 자동 slug 도출 → _inputs/{slug}.md 자동 생성
#   python3 fetch_competitor_ads.py medicube       # 기존 시드 슬러그
#   python3 fetch_competitor_ads.py                # 전체 시드 일괄
#   python3 fetch_competitor_ads.py --max 50       # 브랜드당 최대 광고 수
#   python3 fetch_competitor_ads.py --no-gemini    # 분석 스킵 (수집만)
#
# 출력
#   02_competitor/{slug}/
#     ├── ad-creatives.md          # 분석 요약 (단일 브랜드)
#     └── ad-creatives/
...
from competitor_inputs import (
    COMBINED_INPUT,
    INPUTS_DIR,
    load_combined_competitors,
    load_competitor,
    load_competitors,
    slugify,
    write_seed_md,
...
COMPETITORS_ROOT = ROOT / "02_competitor"
...
# 워크스페이스 통합 자격증명 파일 자동 로드 (09_tracking/.env)
_load_env_file(ROOT / "09_tracking" / ".env")
...
# ──────────────────────── Gemini 5단계 분석 프롬프트 ────────────────────────
# 레퍼런스 분석법 (02_competitor/_reference/reference_analysis_method.md) 적용

VIDEO_PROMPT = """당신은 15년차 퍼포먼스마케터이자 뷰티/스킨케어 영상 광고 전문가입니다.
...

{
  ...
  "analysis": {
    ...
    "creative_key_visual": "메인 비주얼 1~2줄. 제품 단독·시연·라이프스타일·인포그래픽·신뢰 자산 중 어떤 형태"
  },
  "_method": "keyframes"
}

## full_storyline 역할 태그
(HOOK) (일상 공감) (페인포인트) (브랜드 USP) (기능/성분) (효과/결과) (사회적 증거) (추천/후기) (CTA) (기타)
...
IMAGE_PROMPT = """당신은 15년차 퍼포먼스마케터이자 뷰티/스킨케어 광고 전문가이며, AI 이미지 생성 프롬프트 전문가입니다.

이 광고 이미지를 **픽셀 단위로 정밀 분석**하고, USP 분석법 (3 항목 + Creative Key Visual + ad_pattern) 에 따라 구조화하세요. 또한 AI 이미지 생성 도구(Gemini/NanoBanana)로 **거의 동일하게 재현**할 수 있는 상세 프롬프트를 작성하세요.

## ad_pattern (광고 유형 1개 라벨)
- "default": 페인 후킹 → Solution → Promotion 약 (가장 흔함)
- "trust_anchor": User's Problem 약, Solution(권위·랭킹·인증) 강 — 페인 없이 신뢰 한 방 (예: 올리브영 1등 + 별점)
- "promotion_anchor": Promotion 거의 전부 — 할인·1+1·한정 강조 (BOFU)
- "hybrid": 2개 이상 동시 강조
...

{
  ...
  "feature_desc": "제품 성분, 효능, 기능 관련 텍스트 (원문 그대로)",
  "empathy_copy": "타겟의 고민, 공감을 자극하는 문구 (원문 그대로)",
  "cta": "할인율, 가격, 구매 유도 문구 (원문 그대로)",
  ...
  "analysis": {
    ...
    "promotion": "지금 구매해야 하는 이유 — 가격 혜택 + 리스크 제거 + 시급성. 실제 카피 인용",
    "creative_key_visual": "메인 비주얼 1~2줄 묘사. 변주 — 제품 단독 / 시연 / 라이프스타일 / 인포그래픽 / 신뢰 자산 중 어떤 형태"
  },
...
def _urlopen_or_quota(req, timeout):
    """urlopen 호출 중 429/403(quota) 발견 시 GeminiQuotaExceeded 로 변환."""
    try:
        ...
        try:
            ...
            pass
        if _is_quota_error(e.code, body_str):
            raise GeminiQuotaExceeded(f"HTTP {e.code}: {body_str[:200]}")
        raise
...
def build_analysis_md(brand_name_kr, slug, items, query_url, run_id, fmt_counts, processed, analysis_cache=None):
    ...
    lines = []
    lines.append("---")
    lines.append("file_type: competitor-ads")
    lines.append(f"competitor_slug: {slug}")
    ...
    lines.append("")
    lines.append("> 자동 갱신: `02_competitor/_scripts/fetch_competitor_ads.py`")
    ...
    if words:
        ...
        lines.append("- (캡션 없음)")
    lines.append("")
    lines.append("## 7. 광고 카드 전수 (시작일 내림차순)")
    lines.append("")
    by_date = sorted(
        items,
        ...
        if analysis_cache and ad_id in analysis_cache:
            ...
            if a.get("error"):
                ...
                if fmt == "video":
                    ...
                    if summ:
                        lines.append(f"- **Hook**: {summ.get('hook_type', '?')} (점수 {summ.get('hook_effectiveness', '?')}/10)")
                        lines.append(f"- **구조**: {summ.get('structure_type', '?')} · 스타일: {summ.get('video_style', '?')}")
                        lines.append(f"- **종합 점수**: {summ.get('overall_score', '?')}/10")
...
# (각 angle 의 패턴은 광고 1건의 통합 텍스트에 1개 이상 매치되면 점유로 인정)
MESSAGE_ANGLES = [
...
def _extract_usp_dict(entry):
    ...
    Gemini 응답이 한 단계 더 nested 된 케이스(`analysis.analysis.users_problem`)를 우선 처리.
    """
...
def _message_angle_matrix(brands_data):
    """경쟁사 × 메시지 각도 매트릭스. angle 별 매칭 광고 수."""
...
def _extract_cta_library(brands_data):
    """경쟁사별 CTA 문구 라이브러리 — 버튼 / 가격 혜택 / 리스크 제거."""
...
def gemini_generate_text(api_key, prompt, timeout=120):
    """텍스트 only Gemini 호출 (빈틈 합성용). 실패 시 None 반환."""
    ...
    try:
        res = http("POST", url, payload=body, timeout=timeout)
        return res["candidates"][0]["content"]["parts"][0]["text"]
    except Exception as e:
        return f"(Gemini 호출 실패: {e})"


def _synthesize_whitespace(brand_brief_text, matrix_labels, matrix, pattern_summary, gemini_key):
    """자사 빈틈 합성 — Gemini 1콜 또는 결정론적 폴백.

    결정론적: 모든 경쟁사가 0인 angle 나열.
    Gemini: brand_brief + 매트릭스 + ad_pattern 분포 → 빈틈 3-5개 + USP 매핑 표.
    """
    ...
    if not gemini_key or not brand_brief_text:
        out = ["**경쟁사 전체가 0건인 메시지 각도 (결정론적 추출):**"]
        if zero_angles:
            for a in zero_angles: out.append(f"- {a}")
        else:
            out.append("- (없음 — 경쟁사가 모든 각도를 일부라도 점유)")
        out.append("")
        out.append("**점유율 최하위 angle Top 3 (저점유 빈틈):**")
        for lbl in low_top3: out.append(f"- {lbl}")
        out.append("")
        out.append("> ⚠️ Gemini 키 또는 `01_brand/brand_brief.md` 미발견 — 자사 USP 매핑은 수기 추가 필요.")
        return "\n".join(out)

    matrix_summary = []
    for lbl in matrix_labels:
        cells = [f"{slug}={matrix[slug].get(lbl, 0)}" for slug in matrix]
        matrix_summary.append(f"  - {lbl}: {' / '.join(cells)}")
    pattern_str = " / ".join(f"{k}={v}" for k, v in pattern_summary.items())

    prompt = f"""당신은 15년차 퍼포먼스마케터. 자사 brand_brief 와 경쟁사 광고 분석 결과를 받아,
자사가 비집고 들어갈 "빈틈" (White Space) 을 마크다운으로 정리하세요.

# 자사 brand_brief (발췌)
{brand_brief_text[:3500]}

# 경쟁사 메시지 각도 매트릭스 (각도별 점유 광고 수)
{chr(10).join(matrix_summary)}

# 경쟁사 ad_pattern 분포 (광고 유형)
{pattern_str}

# 결정론적으로 추출된 '0건' angle
{', '.join(zero_angles) if zero_angles else '(없음)'}

# 작성 요구
다음 3개 헤더로 마크다운만 출력 (한국어, 다른 설명·전후 코멘트·코드펜스 ❌):

### 메시지 각도 빈틈 (3-5개)
- **각도명** — 자사 USP A/B/C 중 어느 것에 매핑되는지 + 1줄 근거

### CTA 문구 빈틈 (2-3개)
- 경쟁사가 안 쓰는 CTA 문법 + 자사가 쓸 수 있는 이유 (1줄)

### 자사 USP 3개 × 빈틈 매핑 표
| 자사 USP | 비어있는 angle | 추천 카피 시드 |
|---|---|---|
"""
    text = gemini_generate_text(gemini_key, prompt) or ""
    return text.strip() or "(Gemini 응답 비어있음 — 수기 추가 필요)"


# ──────────────────────── 통합 트렌드 리포트 ────────────────────────

def build_trend_report(brands_data, gemini_key=None, brand_brief_path=None):
    """경쟁사별 분석 데이터를 SKILL Step 7 골격(9 섹션) 으로 통합.

    brands_data: [{slug, brand_name_kr, items, cache}]
    gemini_key: 빈틈(White Space) 섹션 Gemini 합성용. 없으면 결정론적 폴백.
    brand_brief_path: 자사 brand_brief.md 경로. 없으면 빈틈은 결정론적 폴백만.
    """
    today = dt.datetime.now().strftime("%Y-%m-%d %H:%M KST")
    out = []
    out.append("---")
    out.append("file_type: competitor-ads-trend")
    out.append(f"last_updated: {today.split()[0]}")
    out.append(f"competitor_count: {len(brands_data)}")
    out.append("---")
    out.append("")
    out.append("# 경쟁사 광고 트렌드 (전체 통합)")
    out.append("")
    out.append(f"> 자동 갱신 — {today}")
    out.append("> 매 실행 시 덮어쓰기됩니다.")
    out.append("")
    ...
    # ── § 2. 강조 메시지 매트릭스 ──
    out.append("## 2. 강조 메시지 매트릭스 (경쟁사 × 메시지 각도)")
    out.append("")
    ...
    out.append("")
    if total_patterns:
        top_pat = total_patterns.most_common(1)[0]
        out.append(f"> 💡 경쟁사 광고의 **{top_pat[1]/total*100:.0f}%** 가 `{top_pat[0]}` — 자사가 다른 패턴으로 변주하면 차별화 여지.")
        out.append("")

    # ── § 4. USP 3 항목 가로 비교 ──
    out.append("## 4. USP 3 항목 + Creative Key Visual — 경쟁사 가로 비교")
    out.append("")
    out.append("> USP 분석법 (User's Problem · Solution · Promotion + Creative Key Visual) 으로 분해된 결과를 항목별 가로 비교. 경쟁사별 상위 3개 인용.")
    out.append("")
    stage_keys = [
    ...
    # ── § 5. 카피라이팅 Top 인용 (압축) ──
    out.append("## 5. 카피라이팅 Top 인용 (경쟁사별 상위 5개)")
    out.append("")
    out.append("> 광고 카피 raw 덤프 ❌. 경쟁사별 메인 카피 중복 제거 후 상위 5개만 인용. 전체는 `{slug}/ad-creatives.md` 참고.")
    out.append("")
    for b in brands_data:
    ...
    # ── § 6. CTA 문구 라이브러리 ──
    out.append("## 6. CTA 문구 라이브러리 (경쟁사별)")
    out.append("")
    out.append("| 경쟁사 | 대표 버튼 | 가격 혜택 (Top 3) | 리스크 제거 (Top 3) |")
    out.append("|---|---|---|---|")
    ...
    # ── § 8. 빈틈 (White Space) ──
    out.append("## 8. 빈틈 (자사가 비집고 들어갈 White Space)")
    out.append("")
    brand_brief_text = ""
    if brand_brief_path:
        try:
            p = pathlib.Path(brand_brief_path)
            if p.exists(): brand_brief_text = p.read_text(encoding="utf-8")
        except Exception: pass
    pattern_summary = dict(total_patterns)
    out.append(_synthesize_whitespace(brand_brief_text, labels, matrix, pattern_summary, gemini_key))
    out.append("")

    # ── § 9. 다음 모니터링 트리거 ──
    out.append("## 9. 다음 모니터링 트리거")
    out.append("")
    out.append("- [ ] 경쟁사 신규 활성 광고 5건 이상 → `python3 02_competitor/_scripts/fetch_competitor_ads.py` 재실행")
    out.append("- [ ] 새 키워드 / angle 등장 (예: 새 임상 수치·신규 콜라보) → 매트릭스 업데이트")
    out.append("- [ ] 기존 광고 절반 이상 비활성 → 직전 트렌드와 diff 비교")
    out.append("- [ ] 자사 캠페인 ROAS 하락 + 경쟁사 신규 promotion_anchor 증가 → 가격 대응 검토")
    out.append("")
...
def fetch_brand(token, actor, slug, brand_name_kr, ad_library_url, max_ads, gemini_key=None, rpm_delay=4.0,
                inputs_dir=None, auto_seed=False, gemini_tier="free"):
    """단일 브랜드 크롤 + 분석 + 보고서 생성.
    ...
    if auto_seed and items:
        ...
        seed_path = write_seed_md(slug, brand_name_kr, ad_library_url, inputs_dir=inputs_dir)
        if seed_path:
    ...
    out_dir = COMPETITORS_ROOT / slug / "ad-creatives"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    if gemini_key:
        ...
        for ad_id, mi in media_index.items():
            ...
            try:
                ...
                analysis_path.write_text(json.dumps(cache, ensure_ascii=False, indent=2), encoding="utf-8")
                quota_hit = True
                # 남은 광고는 cache 에 안 적음 → 다음 실행 시 자연스럽게 재시도
                ...
                print(f"     ② 즉시 완료가 필요하면: 09_tracking/.env 에 GEMINI_API_KEY_PAID=AQ... 추가 후 `--paid` 플래그로 재실행")
                break
            except Exception as e:
...
def rebuild_md_only(slug, brand_name_kr):
    base = COMPETITORS_ROOT / slug / "ad-creatives"
...
def main():
    parser = argparse.ArgumentParser(
        description="Apify로 경쟁사 Meta 광고 소재 수집 + 5단계 분석 (URL 1개로 시작)",
    ...
    parser.add_argument(
        ...
        nargs="?",
        help=(
            "Meta Ad Library URL (https://...) 또는 기존 시드 슬러그 (예: medicube). "
    ...
    rows = []          # 일괄·슬러그 흐름 입력
    url_targets = []   # URL-only 흐름 입력 [(url, slug_or_None, brand_kr_or_None)]

    if is_url:
        url_targets.append((target, None, None))
    elif target:
        # 슬러그
        row = load_competitor(target, inputs_dir=INPUTS_DIR)
        if not row:
            sys.exit(f"slug 매칭 없음: _inputs/{target}.md 가 없습니다")
        rows = [row]
    else:
        # 통합 입력 파일 우선 (사용자가 편집하는 유일한 파일)
        combined = load_combined_competitors()
        if combined:
            ...
            for c in combined:
                ...
                url_targets.append((c["url"], None, kr_hint or None))
        else:
            rows = load_competitors(inputs_dir=INPUTS_DIR)

        if not rows and not url_targets:
            sys.exit(
                f"입력 없음: {COMBINED_INPUT.relative_to(ROOT)} 에 URL 을 적거나, "
                f"URL 을 직접 인자로 넘기세요.\n"
                f"  예) python3 fetch_competitor_ads.py 'https://www.facebook.com/ads/library/?...&view_all_page_id=123'"
            )

    # 2) md-only 분기 — 경쟁사별 ad-creatives.md + 통합 competitor_ads.md 동시 재생성
    if args.md_only:
        # Step A) rebuild 대상 slug 모으기 — rows (옛 시드) + 02_competitor/*/ad-creatives/metadata.json 자동 탐지
        seen_slugs = set()
        rebuild_targets = []  # [(slug, brand_kr)]
        for r in rows:
            slug = r["competitor_slug"]
            if slug and slug not in seen_slugs:
                seen_slugs.add(slug)
                rebuild_targets.append((slug, r["brand_name_kr"]))
        # 통합 입력 모드/디렉토리 폴백 — metadata.json 있는 모든 slug
        for meta_path in COMPETITORS_ROOT.glob("*/ad-creatives/metadata.json"):
            slug = meta_path.parent.parent.name
            if slug.startswith("_") or slug in seen_slugs:
                continue
            seen_slugs.add(slug)
            # brand_kr: _inputs/{slug}.md 에서 시도, 실패 시 slug 그대로
            seed_row = load_competitor(slug, inputs_dir=INPUTS_DIR)
        ...
        # Step B) 통합 competitor_ads.md 재생성 — gemini_key·brand_brief 로드
        gemini_key = (
            os.environ.get("GEMINI_API_KEY_PAID", "").strip() if args.paid
            else os.environ.get("GEMINI_API_KEY_FREE", "").strip()
        ) or os.environ.get("GEMINI_API_KEY", "").strip() or None

        if brands_data:
            brand_brief_path = ROOT / "01_brand" / "brand_brief.md"
            trend_md = build_trend_report(
                brands_data,
                gemini_key=gemini_key,
                brand_brief_path=brand_brief_path if brand_brief_path.exists() else None,
            )
            trend_path = COMPETITORS_ROOT / "competitor_ads.md"
            trend_path.write_text(trend_md, encoding="utf-8")
            print(f"\n통합 트렌드 리포트: {trend_path.relative_to(ROOT)}")
        print("\n완료 (md 재생성).")
        return
    ...
    if not token:
        sys.exit(
            "APIFY_TOKEN 이 없습니다.\n"
            "  ① 권장: `09_tracking/.env` 에 한 줄 추가 → APIFY_TOKEN=apify_api_...\n"
            "  ② 임시: 현재 셸에서 export APIFY_TOKEN='apify_api_...'\n"
    ...
    # Gemini 키 선택 — 무료/유료 듀얼 키 지원
    #   --paid 플래그 → GEMINI_API_KEY_PAID 사용 (없으면 GEMINI_API_KEY 폴백)
    #   디폴트(무료) → GEMINI_API_KEY_FREE 우선, 없으면 GEMINI_API_KEY 폴백
    gemini_key = None
    gemini_tier = "free"
    if not args.no_gemini:
        if args.paid:
            ...
            if not gemini_key:
                sys.exit(
                    ...
                    "  09_tracking/.env 에 한 줄 추가 → GEMINI_API_KEY_PAID=AQ..."
                )
        else:
        ...
        if not gemini_key:
            ...
            print("   09_tracking/.env 에 GEMINI_API_KEY_FREE=AQ... (무료) 또는 GEMINI_API_KEY=AQ... 한 줄 추가")
    ...
    # 4) URL-only 흐름
    for url, slug, kr in url_targets:
        try:
            res = fetch_brand(
    ...
    if brands_data:
        # 단일 브랜드만 처리한 경우에도, _inputs 의 다른 슬러그도 분석 캐시가 있으면 합쳐서 트렌드를 봅니다
        all_rows = load_competitors(inputs_dir=INPUTS_DIR)
        seen = {b["slug"] for b in brands_data}
        for r in all_rows:
            if r["competitor_slug"] in seen:
                continue
            base = COMPETITORS_ROOT / r["competitor_slug"] / "ad-creatives"
            meta_path = base / "metadata.json"
            if not meta_path.exists():
                continue
            try:
