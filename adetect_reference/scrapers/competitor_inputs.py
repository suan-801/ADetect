...
#
# 입력
#   02_competitor/_inputs/{slug}.md
#
# 사용 (다른 스크립트에서)
#   from competitor_inputs import load_competitors, INPUTS_DIR
#   rows = load_competitors()                     # status=paused 제외
#   rows = load_competitors(include_paused=True)
#   row = load_competitor("medicube")             # 단일
...
INPUTS_DIR = ROOT / "02_competitor" / "_inputs"
COMBINED_INPUT = INPUTS_DIR / "competitors.md"  # 통합 입력 파일 (사용자가 편집하는 유일한 곳)
...
def parse_competitor_md(path):
    ...
    반환 키:
      competitor_slug, brand_name_kr, brand_name_en, product_name_kr,
      classification, status, meta_ad_library_link, notes, source_path
    """
    ...
    slug = (fm.get("competitor_slug") or path.stem).strip()
    ...
    return {
        ...
        "classification": (fm.get("classification") or "").strip().lower(),
...
def load_competitors(include_paused=False, inputs_dir=None):
    ...
    for path in sorted(base.glob("*.md")):
        ...
        if name in {"readme", "template", "competitor-template", "competitors"}:
            continue
        try:
            row = parse_competitor_md(path)
        except Exception as e:
            print(f"  ⚠️ 파싱 실패: {path.name} — {e}", file=sys.stderr)
            continue
        if not row["competitor_slug"]:
...
def load_competitor(slug, inputs_dir=None):
    base = pathlib.Path(inputs_dir) if inputs_dir else INPUTS_DIR
    candidate = base / f"{slug}.md"
    if not candidate.exists():
        return None
    return parse_competitor_md(candidate)


# ──────────────────────── 통합 입력 파일 (competitors.md) ────────────────────────

# `## 직접경쟁` / `## 간접경쟁` / `## indirect` 같은 헤딩 → classification
_CLASSIFICATION_MAP = {
    "직접경쟁": "직접경쟁",
    "직접": "직접경쟁",
    "direct": "직접경쟁",
...
def _classify_heading(title):
    t = title.strip().lower()
    return _CLASSIFICATION_MAP.get(t, "")


def load_combined_competitors(path=None):
    """`_inputs/competitors.md` 한 파일에서 URL 리스트 추출.

    포맷 규칙:
      - `## 직접경쟁` / `## 간접경쟁` 헤딩 → 다음 URL 들의 classification
      - URL 라인: `https://...` (옵션: 뒤에 `# 메모` — brand_kr 힌트)
      - URL 앞에 `#` 가 붙어있으면 (주석 처리) → 스킵
      - `>` 인용·일반 주석 라인·빈 줄은 무시

    반환: list of {"url", "classification", "notes"}
    """
...
def write_seed_md(slug, brand_name_kr, ad_library_url, brand_name_en="", inputs_dir=None):
...
if __name__ == "__main__":
    ...
    if combined:
        print(f"통합 입력 (competitors.md): {len(combined)}개 URL\n")
        for c in combined:
            print(f"  [{c['classification'] or '미분류'}] {c['notes'][:60] or '(메모 없음)'}")
            print(f"     {c['url'][:120]}...")
        print()
    ...
    if rows:
        ...
        for r in rows:
            print(f"[{r['competitor_slug']}] {r['brand_name_kr']} ({r['brand_name_en']}) — {r['classification']} / {r['status']}")
