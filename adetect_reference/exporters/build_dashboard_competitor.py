#!/usr/bin/env python3
# 경쟁사 광고 모니터링 대시보드 — 정적 HTML + 로컬 미디어
#
# 입력
#   02_competitor/{slug}/ad-creatives/{metadata.json, analysis.json, images/, videos/}
#
# 출력
#   02_competitor/dashboard/
...
from competitor_inputs import load_competitors

ROOT = pathlib.Path(__file__).resolve().parents[2]
COMPETITORS_ROOT = ROOT / "02_competitor"
DASHBOARD = COMPETITORS_ROOT / "dashboard"
INPUTS_DIR = COMPETITORS_ROOT / "_inputs"
...
def copy_media(slug):
    src_root = COMPETITORS_ROOT / slug / "ad-creatives"
...
def main():
    parser = argparse.ArgumentParser(description="경쟁사 광고 모니터링 대시보드 빌드")
    parser.add_argument("--no-copy", action="store_true", help="미디어 재복사 스킵")
    args = parser.parse_args()

    rows = load_competitors(inputs_dir=INPUTS_DIR)
    if not rows:
        sys.exit(f"시드 없음: {INPUTS_DIR}/{{slug}}.md 를 1개 이상 만들거나, fetch_competitor_ads.py 로 URL 을 직접 처리하세요")
    ...
    if not brands:
        sys.exit("로드 가능한 브랜드 데이터가 없습니다. fetch_competitor_ads.py 부터 실행하세요.")
