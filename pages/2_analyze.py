...
# ── STEP 1: 확인 화면 (AI 추천 → 사용자 확인, §9) ──────────────────────
    ...
    st.markdown("#### STEP 1 · 브랜드명을 기반으로 다음 정보를 추천했습니다")
    ...
    with st.container(border=True):
        ...
        st.write("경쟁사 (사용자가 직접 입력했다면 그 값이 우선 반영됨, §7-10)")
        selected = []
        for c in candidates:
        ...
        if col1.button("이전", key="btn_confirm_back"):
            st.session_state.step = "input"
            st.rerun()
        if col2.button("확정하고 Workspace로 이동", type="primary", key="btn_confirm_go"):
...
# ── STEP 2: 분석 Workspace (5개 탭) ─────────────────────────────────────
    ...
    strip = (
        ...
        f"<b>{session['brand_name']}</b> · {session['category']} · {target_display}"
        f"<span class='adetect-chip'>시장 {status_icon(session['market_status'])}</span>"
        f"<span class='adetect-chip'>타겟 {target_icon}</span>"
        f"<span class='adetect-chip'>브랜드 {status_icon(session['brand_status'])}</span>"
        f"<span class='adetect-chip'>소재 {status_icon(session['creative_status'])}</span>"
        f"<span class='adetect-chip'>종합 {synthesis_icon}</span>"
        f"</div>"
    )
